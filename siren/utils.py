...
def densely_sample_activations(model, num_dim=1, num_steps=int(1e6)):
    ...
    if num_dim == 1:
        ...
        input = torch.stack(
            torch.meshgrid(*(input for _ in num_dim)),
            dim=-1).view(-1, num_dim)
...
def write_wave_summary(model,
    ...
    with torch.no_grad():
        ...
        coords = [
            dataio.get_mgrid((1, sl, sl), dim=3)[None, ...].cuda()
            for f in frames
        ]
...
def write_helmholtz_summary(model,
    ...
    if 'gt' in gt:
        ...
        writer.add_image(
            prefix + 'gt_real',
            make_grid(
                gt_field[..., 0, :, :], scale_each=False, normalize=True),
            global_step=total_steps)
        writer.add_image(
            prefix + 'gt_imaginary',
            make_grid(
                gt_field[..., 1, :, :], scale_each=False, normalize=True),
            global_step=total_steps)
        writer.add_image(
            prefix + 'gt_angle',
...
def write_image_summary_small(image_resolution,
                              mask,
                              model,
                              model_input,
                              gt,
    ...
    hypernet_activation_summary(model, model_input, gt, model_output, writer,
                                total_steps, prefix)
...
def make_contour_plot(array_2d, mode='log'):
    ...
    if (mode == 'log'):
        ...
        levels = np.concatenate((levels_neg, np.zeros((0)), levels_pos),
                                axis=0)
        colors = plt.get_cmap("Spectral")(
            np.linspace(0., 1., num=num_levels*2 + 1))
    elif (mode == 'lin'):
        num_levels = 10
...
def write_sdf_summary(model,
    ...
    with torch.no_grad():
        yz_slice_coords = torch.cat(
            ...
            dim=-1)
        yz_slice_model_input = {
            'coords': yz_slice_coords.to(device)[None, ...]
        }
        ...
        xz_slice_coords = torch.cat(
            ...
            dim=-1)
        xz_slice_model_input = {
            'coords': xz_slice_coords.to(device)[None, ...]
        }
        ...
        xy_slice_coords = torch.cat(
            ...
            dim=-1)
        xy_slice_model_input = {
            'coords': xy_slice_coords.to(device)[None, ...]
        }
...
def hypernet_activation_summary(model,
                                model_input,
                                gt,
                                model_output,
                                writer,
    ...
    with torch.no_grad():
        ...
        for name, param in hypo_parameters.items():
            writer.add_histogram(
                prefix + name, param.cpu(), global_step=total_steps)

        writer.add_histogram(
            prefix + 'latent_code', embedding.cpu(), global_step=total_steps)


def write_video_summary(vid_dataset,
                        model,
                        model_input,
                        gt,
                        model_output,
    ...
    with torch.no_grad():
        coords = [
            dataio.get_mgrid((1, resolution[1], resolution[2]),
                             dim=3)[None, ...].cuda() for f in frames
        ]
        for idx, f in enumerate(frames):
    ...
    pred_vid = output.view(len(frames), resolution[1], resolution[2],
                           3) / 2 + 0.5
...
def write_image_summary(image_resolution,
    ...
    write_psnr(
        ...
        dataio.lin2img(gt['img'], image_resolution), writer, total_steps,
        prefix + 'img_')


def write_laplace_summary(model,
                          model_input,
                          gt,
                          model_output,
                          writer,
    ...
    output_vs_gt = torch.cat(
        (dataio.rescale_img(gt_img), dataio.rescale_img(pred_img, perc=1e-2)),
        dim=-1)
    writer.add_image(
    ...
    pred_laplace = diff_operators.laplace(model_output['model_out'],
                                          model_output['model_in'])
    ...
    gt_laplace_img = cv2.applyColorMap(gt_laplace_img.squeeze(),
                                       cmapy.cmap('RdBu'))
    ...
    # Plot pred gradient
    pred_gradients = diff_operators.gradient(model_output['model_out'],
                                             model_output['model_in'])
    ...
    pred_laplace_img = cv2.applyColorMap(pred_laplace_img.squeeze(),
                                         cmapy.cmap('RdBu'))
...
def write_gradients_summary(model,
                            model_input,
                            gt,
                            model_output,
                            writer,
...
def write_gradcomp_summary(model,
                           model_input,
                           gt,
                           model_output,
                           writer,
...
def write_audio_summary(logging_root_path,
                        model,
                        model_input,
                        gt,
                        model_output,
    ...
    coords = torch.squeeze(
        model_output['model_in'].clone()).detach().cpu().numpy()
    ...
    # write audio files:
    wavfile.write(
        os.path.join(logging_root_path, 'gt.wav'), gt_rate,
        gt_func.detach().cpu().numpy())
    wavfile.write(
        os.path.join(logging_root_path, 'pred.wav'), gt_rate,
        pred_func.detach().cpu().numpy())


def min_max_summary(name, tensor, writer, total_steps):
    writer.add_scalar(name + '_min',
                      tensor.min().detach().cpu().numpy(), total_steps)
    writer.add_scalar(name + '_max',
                      tensor.max().detach().cpu().numpy(), total_steps)
