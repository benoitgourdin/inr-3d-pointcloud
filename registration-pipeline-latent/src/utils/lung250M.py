...
class Lung250MDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, cfg, points_per, phase, split, pair=0):
        self.is_train = True if phase == 'train' else False
        self.is_val = True if phase == 'val' else False
        self.split = split
        self.points_per = points_per

        if self.split == 'val':
            self.pcd_template = os.path.join(
                '/vol/aimspace/users/gob/Dataset/Lung250M-4B/cloudsTs/coordinates',
                'case_{:03d}_{}.pth')
            self.gt_template = os.path.join(
                '/vol/aimspace/users/gob/Dataset/Lung250M-4B/cloudsTs/coordinates',
                'case_{:03d}.pth')
        else:
            self.pcd_template = os.path.join(
                '/vol/aimspace/users/gob/Dataset/Lung250M-4B/cloudsTr/coordinates',
                'case_{:03d}_{}.pth')
            self.gt_template = os.path.join(
                '/vol/aimspace/users/gob/Dataset/Lung250M-4B/cloudsTr/coordinates',
                'case_{:03d}.pth')
        self.idx_16k = torch.load(
            '/vol/aimspace/users/gob/Dataset/Lung250M-4B/all_ind_16384.pth',
            map_location='cpu',
            weights_only=True)

        test_cases = np.array([0, 1, 3, 4, 5, 6, 7, 9, 10, 11])

        # test_cases = np.array([0, 1])

        if split == 'train':  # 87 pairs
            self.case_list = np.arange(104)
            no_cases = np.concatenate((np.array([2, 8, 54, 55, 56, 94,
                                                 97]), test_cases))
            self.case_list = self.case_list[~np.isin(self.case_list, no_cases)]

            # self.case_list = np.array([3, 4])

        elif split == 'val':  # 4 pairs

            # self.case_list = np.array([2, 8])

            self.case_list = np.array([2, 8, 94, 97])
        elif split == 'test':  # 10 pairs
            self.case_list = test_cases
        else:
    ...
    def __getitem__(self, idx):
        ...
        pcd_tgt = torch.load(
            pcd_template.format(case, 1), weights_only=True)[2]
        pcd_src = torch.load(
            pcd_template.format(case, 2), weights_only=True)[2]
        ...
        corrfield_flow = torch.load(
            gt_template.format(case), weights_only=True)['cloud_gt_mov']
        ...
        factor = std_src / std_tgt * self.norm_factor
        ...
        if self.is_train:
            if self.augm_setting.METHOD == 'multiscale_local_global':
                ...
                local_control_idx = np.random.permutation(
                    pcd.shape[0])[:num_control_points_local]
                local_control_shifts = np.random.uniform(
                    -1., 1.,
                    (num_control_points_local, 3)) * max_control_shift_local
                local_control_pts = pcd[local_control_idx]
                sq_dist = np.sum(
                    np.square(pcd[:, None] - local_control_pts[None]), axis=2)
                weights = np.exp(-0.5 * sq_dist / kernel_std_local**2)
                local_pcd_shifts = np.sum(
                    weights[:, :, None] * local_control_shifts[None],
                    axis=1) / np.sum(
                        weights[:, :, None], axis=1)
                ...
                global_control_shifts = np.random.uniform(
                    -1, 1., (global_control_pts.shape[0],
                             3)) * max_control_shift_global
                sq_dist = np.sum(
                    np.square(pcd_augm[:, None] - global_control_pts[None]),
                    axis=2)
                weights = np.exp(-0.5 * sq_dist / kernel_std_global**2)
                global_pcd_shifts = np.sum(
                    weights[:, :, None] * global_control_shifts[None],
                    axis=1) / np.sum(
                        weights[:, :, None], axis=1)
                ...
                scale = np.random.uniform(1 - scale_offset, 1 + scale_offset,
                                          (1, 3))
                rot_angles = np.deg2rad(
                    np.random.uniform(-rot_max, rot_max, 3))
                ...
                rot_mat_x = np.array([[1, 0, 0],
                                      ...
                                      [0, np.sin(theta),
                                       np.cos(theta)]])
                theta = rot_angles[1]
                rot_mat_y = np.array([[np.cos(theta), 0,
                                       np.sin(theta)], [0, 1, 0],
                                      [-np.sin(theta), 0,
                                       np.cos(theta)]])
                theta = rot_angles[2]
                rot_mat_z = np.array([[np.cos(theta), -np.sin(theta), 0],
                                      [np.sin(theta),
        ...
        indices = np.arange(pcd_src.shape[0])  # Create an array of indices
        np.random.shuffle(indices)
        pcd_src = pcd_src[indices]
        gt_flow = gt_flow[indices]
        result_dict = {
            'moving_pc':
                np.float32(pcd_src)[:int(self.points_per * pcd_src.shape[0])],
            'registration_flow':
                np.float32(gt_flow)[:int(self.points_per * pcd_src.shape[0])],
            'scaling_factor':
                factor,
            'caseids':
                self.case_list[idx]
